...
def ptrac_reduce(analysisdict, reduced_with_origin=False):
    newdict = {}
    if reduced_with_origin == False :
        reducekey = ["cell", "Nhist"]
    else :
        reducekey = ["cell", "Nhist", "origin"]
    for key, df in analysisdict.items():
        newdict[key] = df.drop_duplicates(subset=reducekey, keep="first", ignore_index=True)
    return newdict
...
def ptrac_rossi(df, shakelimit = 100):
    ...
    while True :
        ts = tdata[i:]-tdata[:-i]
        if (ts<=shakelimit).any():
            result.append(ts[ts<=shakelimit])
            i+=1
        else :
            break
...
def read_jjh(jjhdatafolder, inid=1, tid=None, stage=0):
    ...
    if tid == None :
        ...
        for tt in range(1, 21):
            ...
            df_n, df_p = read_jjh(jjhdatafolder, inid, tt, stage)
            dfs[fname_n[:-2]]=df_n
            dfs[fname_p[:-2]]=df_p
        return dfs
